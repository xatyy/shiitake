...
public class ProductSpecifications {
    public static Specification<Product> byFilter(Long storeId, Map<String, String> filterMap) {
            ...
            filterMap.forEach((key, value) -> {
                if ("productName".equals(key) && value != null && !value.trim().isEmpty()) {
                    String escapedValue = StringUtils.replace(value.trim(), "!", "!!")
                            ...
                            .replace("_", "!_")
                            .replace("[", "![");
                    predicates.add(criteriaBuilder.like(criteriaBuilder.lower(root.get("productName")), "%" + escapedValue.toLowerCase() + "%", '!'));
                }
                switch (key) {
                    case "bagType":
